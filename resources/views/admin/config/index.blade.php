...
@section('content')
<div class="space-y-6">
    @foreach($configs as $categoryName => $items)
    <div class="bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 overflow-hidden">
        <div class="px-5 py-4 bg-indigo-900/50 border-b border-white/10 flex items-center gap-2">
            ...
            <h3 class="text-white font-semibold">{{ $categoryName }}</h3>
        </div>
        <form action="{{ route('admin.config.update') }}" method="POST" enctype="multipart/form-data" class="p-5">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="{{ $isWide ? 'sm:col-span-2' : '' }}">
                    <label class="block text-sm font-medium text-indigo-200 mb-1">
                        {{ $config->config_name ?? 'Config #' . $loop->index }}
                    </label>

                    @if($config->type === 'image')
                        <div class="flex items-center gap-3">
                            @if($config->config_value)
                            <img src="{{ asset('uploads/config/' . $config->config_value) }}"
                                 class="w-14 h-14 rounded-xl object-cover border border-white/20"
                                 onerror="this.style.display='none'">
                            @endif
                            <input type="file" name="values[{{ $config->config_name }}]" accept="image/*"
                    ...
                    @elseif($config->type === 'editor')
                        <textarea id="{{ $editorId }}"
                                  name="values[{{ $config->config_name }}]"
                                  class="ck-editor-field">{{ $config->config_value }}</textarea>

                    @elseif($config->type === 'textarea')
                        <textarea name="values[{{ $config->config_name }}]" rows="3"
                            class="w-full bg-white/10 border border-white/20 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-indigo-400 placeholder-indigo-400/50">{{ $config->config_value }}</textarea>

                    @elseif($config->type === 'boolean')
                        <select name="values[{{ $config->config_name }}]"
                            ...
                            <option value="1" {{ $config->config_value == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ $config->config_value == '0' ? 'selected' : '' }}>Non-aktif</option>
                        </select>

                    @elseif($config->type === 'number')
                        <input type="number" name="values[{{ $config->config_name }}]"
                    ...
                    @elseif($config->type === 'date')
                        <input type="date" name="values[{{ $config->config_name }}]"
                            value="{{ $config->config_value }}"
                    ...
                    @elseif($config->type === 'datetime')
                        <input type="datetime-local" name="values[{{ $config->config_name }}]"
                            value="{{ $config->config_value }}"
                            class="w-full bg-white/10 border border-white/20 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-indigo-400 [color-scheme:dark]">

                    @else
                        {{-- input, or any unknown type --}}
                        <input type="text" name="values[{{ $config->config_name }}]"
                            value="{{ $config->config_value }}"
                    ...
                    @if($config->config_description && $config->type !== 'editor')
                    <p class="text-xs text-indigo-400 mt-1">{{ $config->config_description }}</p>
                    @endif
                </div>
                @endforeach
            </div>
