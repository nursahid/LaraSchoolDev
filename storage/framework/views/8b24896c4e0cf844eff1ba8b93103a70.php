<?php $__env->startSection('page-title', 'Tahun Ajaran'); ?>
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <?php $__empty_1 = true; $__currentLoopData = $tapelList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <?php $isAktif = $t->status === '1' || $t->status == 1; ?>
        <div class="bg-white/10 backdrop-blur-sm rounded-2xl border <?php echo e($isAktif ? 'border-green-500/50 ring-1 ring-green-500/30' : 'border-white/20'); ?> hover:bg-white/15 transition relative flex flex-col">
            <div class="p-5 flex-1">
                ...
                <div class="text-xl font-bold text-white mb-1"><?php echo e($t->tahun_ajaran); ?></div>

                <?php if($isAktif && $semesterList->firstWhere('status', '1')): ?>
                <?php $smAktif = $semesterList->firstWhere('status', '1'); ?>
                <div class="text-xs text-green-300 mb-2">
                    <i class="fas fa-calendar-check mr-1"></i><?php echo e($smAktif->semester); ?>

                </div>
                <?php elseif($t->tasm): ?>
                <div class="text-sm text-indigo-300 mb-2"><?php echo e($t->tasm); ?></div>
                <?php endif; ?>

                <div class="text-xs text-indigo-400 space-y-0.5">
                    <div><span class="text-indigo-500">Mulai:</span> <?php echo e($t->tahun_awal ? \Carbon\Carbon::parse($t->tahun_awal)->format('d/m/Y') : '-'); ?></div>
                    <div><span class="text-indigo-500">Selesai:</span> <?php echo e($t->tahun_akhir ? \Carbon\Carbon::parse($t->tahun_akhir)->format('d/m/Y') : '-'); ?></div>
                </div>
            </div>

            <?php if(!$isAktif): ?>
            <div class="px-5 pb-4">
                <form action="<?php echo e(route('admin.tahunajaran.aktifkan', $t->id_tahunajaran)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
    ...
    <div class="bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 overflow-hidden">
        <div class="px-5 py-4 bg-indigo-900/50 border-b border-white/10 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-indigo-400 mt-0.5">
                    Tahun ajaran aktif: <span class="text-indigo-200 font-medium"><?php echo e($tapelAktif->tahun_ajaran); ?></span>
                </p>
            </div>
            <?php $semAktif = $semesterList->firstWhere('status', '1'); ?>
            <?php if($semAktif): ?>
            <span class="text-xs bg-green-500/20 text-green-300 border border-green-500/30 rounded-lg px-3 py-1">
                <i class="fas fa-check-circle mr-1"></i><?php echo e($semAktif->semester); ?>
        ...
        <div class="p-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <?php $__empty_1 = true; $__currentLoopData = $semesterList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php $smIsAktif = $s->status === '1'; ?>
                <div class="rounded-xl border <?php echo e($smIsAktif ? 'border-green-500/50 bg-green-500/10' : 'border-white/15 bg-white/5'); ?> p-4 flex items-center justify-between gap-4 transition">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl <?php echo e($smIsAktif ? 'bg-green-500/20' : 'bg-indigo-900/50'); ?> flex items-center justify-center">
                            <i class="fas <?php echo e($smIsAktif ? 'fa-calendar-check text-green-400' : 'fa-calendar text-indigo-400'); ?>"></i>
                        </div>
                        <div>
                            <div class="text-sm font-medium <?php echo e($smIsAktif ? 'text-green-200' : 'text-indigo-100'); ?>">
                                <?php echo e($s->semester); ?>
                    ...
                    <form action="<?php echo e(route('admin.tahunajaran.semester.toggle', $s->id_semester)); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                        <?php if($smIsAktif): ?>
                        <button type="submit"
...
<div id="modalTambah" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-indigo-950 rounded-2xl border border-white/20 p-6 w-full max-w-md">
        ...
        <form action="<?php echo e(route('admin.tahunajaran.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="space-y-3">
                <div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\www\laravel\laraschool\resources\views/admin/tahunajaran/index.blade.php ENDPATH**/ ?>